...
public class CursosDomainRepository(AppDbContext context) : ICursosDomainRepository
{
    ...
    public async Task<CursoLookupDto> CreateAsync(string nombre, CancellationToken cancellationToken = default)
    {
        var curso = await context.Cursos
            .IgnoreQueryFilters()
        ...
        return new CursoLookupDto { Id = curso.Id, Nombre = curso.Nombre };
    }

    public async Task<CursoLookupDto?> UpdateAsync(int id, string nombre, CancellationToken cancellationToken = default)
    {
